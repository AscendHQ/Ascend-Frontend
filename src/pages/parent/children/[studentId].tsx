    ...
    <section className="rounded-lg border border-border-colour-light bg-white p-5 sm:p-6">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <Summary
          ...
          value={formatCurrency(details.finances.invoiced)}
        />
      ...
      {details.invoices.length === 0 ? (
        <p className="mt-5 text-sm text-Text-meduim-emphasis">
          No invoices yet.
        </p>
      ) : (
      ...
      {details.payments.length > 0 && (
        <div className="mt-6 border-t border-border-colour-light pt-5">
          <h3 className="font-semibold">Recent receipts</h3>
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            {details.payments.slice(0, 6).map(payment => (
              <div
                key={payment._id}
                className="flex justify-between rounded-lg border border-border-colour-light p-3 text-sm"
              >
                <div>
...
function Summary({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="rounded-lg bg-neutral-300 p-4">
      <p className="text-xs text-Text-meduim-emphasis">{label}</p>
      <p className="mt-1 text-xl font-semibold text-Text-high-emphasis">
        {value}
      </p>
    </div>
  );
      ...
      {attendance.history.length > 0 && (
        <div className="mt-5 overflow-x-auto">
          <table className="w-full min-w-[650px] text-left text-sm text-gray-600">
            ...
            <tbody>
              {attendance.history.slice(0, 20).map(item => (
                <tr
      ...
      {details.results.length === 0 ? (
        ...
        <div className="mt-5 space-y-6">
          {details.results.map(result => (
            <div
              key={result._id}
              className="overflow-x-auto rounded-lg border border-border-colour-light"
            >
              <div className="flex justify-between bg-neutral-300 p-4">
                <strong>
                  {result.session}, {result.term}
                </strong>
                <span className="rounded-full bg-success-light px-2.5 py-1 text-xs font-semibold capitalize text-success-dark">
                  {result.status ?? "approved"}
                </span>
              </div>
              <table className="w-full min-w-[650px] text-left text-sm text-gray-600">
                <thead className="text-xs font-semibold text-Text-high-emphasis">
                  <tr>
                ...
                <tbody>
                  {result.blocks.map(block => (
                    ...
                    >
                      ...
                      <td className="p-3">{block.mid_term_test}</td>
                      <td className="p-3">{block.ca_score}</td>
                      <td className="p-3">{block.exam_score}</td>
                      <td className="p-3 font-semibold">{block.total}</td>
                      <td className="p-3 font-semibold">{block.grade}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
  ...
  const childQuery = useQuery({
    ...
    queryFn: () =>
      axiosInstance
        ...
        .then(response => response.data as ParentChildDetails),
  ...
  return (
    <ParentLayout title={studentName}>
      <Link
        ...
        className="mb-5 inline-flex items-center gap-2 font-semibold text-primary-purple-700"
      >
        <Icon icon="material-symbols:arrow-back-rounded" /> Back to my children
      </Link>
      {childQuery.isLoading ? (
        <div className="flex justify-center py-20">
          <Spinner />
        </div>
      ) : childQuery.isError || !childQuery.data ? (
        <div className="rounded-lg border border-secondary-red-500 bg-white p-8 text-secondary-red-600">
          This student information could not be loaded.
        </div>
      ) : (
        <div className="space-y-6">
          <StudentTimetable studentId={studentId} />
          <AttendanceSection details={childQuery.data} />
          <ResultsSection details={childQuery.data} />
          <FinanceSection details={childQuery.data} />
        </div>
      )}
    </ParentLayout>
  );
