...
export default function StudentPortalAccounts() {
  ...
  const mutation = useMutation({
    mutationFn: () => axiosInstance.post("/student-portals", { student_id: studentId, password }).then(response => response.data),
    onSuccess: () => {
      api.success({ message: "Student portal account created", description: "The student can sign in with their registration number and temporary password." });
      setStudentId(""); setPassword(""); setShowForm(false);
  ...
  return (
    <Container headerTitle="Student Portals">
      <main className="min-h-full bg-neutral-300 p-6 lg:p-10">
        ...
        {showForm && <section className="mt-6 rounded-xl border bg-white p-6"><div className="flex justify-between"><div><h2 className="text-xl font-bold">Create student account</h2><p className="text-sm text-gray-800">The selected student&apos;s registration number will be their login ID.</p></div><button type="button" onClick={() => setShowForm(false)}><Icon icon="carbon:close-outline" className="text-2xl" /></button></div><div className="mt-5 grid gap-4 md:grid-cols-2"><label className="text-sm font-semibold">Student<select value={studentId} onChange={event => setStudentId(event.target.value)} className="mt-1 w-full rounded border bg-white p-2 font-normal"><option value="">Select student</option>{eligibleStudents.map(student => <option key={student._id} value={student._id}>{getStudentName(student)} — {student.registration_number}</option>)}</select></label><label className="text-sm font-semibold">Temporary password<input type="password" value={password} onChange={event => setPassword(event.target.value)} className="mt-1 w-full rounded border p-2 font-normal" /><span className="mt-1 block text-xs font-normal text-gray-800">8+ characters with uppercase, lowercase, number, and symbol.</span></label></div><div className="mt-5 flex justify-end"><button type="button" disabled={!studentId || !password || mutation.isPending} onClick={() => mutation.mutate()} className="rounded-lg bg-primary-purple-700 px-6 py-3 font-semibold text-white disabled:opacity-50">{mutation.isPending ? "Creating..." : "Create account"}</button></div></section>}
        <section className="mt-6 rounded-xl border bg-white p-6"><h2 className="text-lg font-bold">Registered student accounts</h2>{profilesQuery.isLoading || studentsQuery.isLoading ? <div className="flex justify-center py-16"><Spinner /></div> : !profilesQuery.data?.length ? <p className="py-12 text-center text-gray-800">No student portal accounts yet.</p> : <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[650px] text-left text-sm"><thead className="bg-grey-50 text-xs uppercase text-gray-800"><tr><th className="p-3">Student</th><th className="p-3">Registration number</th><th className="p-3">Login ID</th></tr></thead><tbody>{profilesQuery.data.map(profile => <tr key={profile._id} className="border-t"><td className="p-3 font-semibold">{getStudentName(profile.student)}</td><td className="p-3">{profile.student.registration_number}</td><td className="p-3 font-semibold">{profile.student.registration_number}</td></tr>)}</tbody></table></div>}</section>
      </main>
    </Container>
  );
}
